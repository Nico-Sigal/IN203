...
int main(int argc, char ** argv)
{
	...
	while (1) 
	{
		...
		mise_a_jour(param, width, height, g.data(), g_next.data()); 	// calcul g_next (paralléliser)
		end1 = std::chrono::system_clock::now();
		g_next.swap(g); 						// mise à jour g
		gr.render(g); 							// affichage
		...
		if (i > 100)
		{
			average1 += elaps1.count()*1000;
			average2 += elaps2.count()*1000;
		};
		//_sleep(1000);
		if (SDL_PollEvent(&event) && event.type == SDL_QUIT) 
		{
			std::cout << std::endl << "Temps de calcul = " 
			          << average1/(i-100);
			std::cout << std::endl << "Temps d'affichage = "  
				  << average2/(i-100) << std::endl;
			break;
		}
	}
